...
function LoginAdmin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/authAdmin/login/${username}/${password}`);
      const data = await response.json();

      if (response.ok) {
        setGlobalUsername(username); // Simpan username ke context
        navigate("/DashboardAdmin"); // Pindah ke dashboard
      } else {
    ...
    <div className="login-wrapper">
      <div className="login-form-container">
        <div className="login-content">
          ...
          />
          <h5 className="login-title">LOGIN ADMIN</h5>
      ...
      <p className="signup-text">
        Belum punya akun? <a href="/RegisterAdmin" className="signup-link">Daftar di sini</a>
      </p>
          </div>
      </div>
    </div>
